...
export class OrderWorkflowRuntime extends WorkflowRuntime<{ orderId: string }> {
  ...
  protected async completion_experimental(event: WorkflowCompletionEvent): Promise<void> {
...
- **`cancel(reason?)`** — Moves to terminal **cancelled** and stops workflow execution. If the runtime defines **`completion_experimental`**, it schedules an alarm for durable completion delivery; otherwise it clears the current alarm.
...
Define the optional protected **`completion_experimental(event)`** method on the runtime to consume terminal workflow outcomes:
...
I think a cleaner design for terminal projection is to keep synchronization out of the definition and implement **`completion_experimental`** on the runtime instead:
